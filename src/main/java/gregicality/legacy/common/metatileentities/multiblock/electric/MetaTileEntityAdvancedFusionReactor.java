...
import gregicality.legacy.api.block.IAdvancedFusionCoilStats;
import gregicality.legacy.api.recipe.builders.AdvancedFusionCoilTierProperty;
...
import static gregicality.legacy.common.metatileentities.GCYLRMetaTileEntities.ADVANCED_FUSION_REACTOR;

public class MetaTileEntityAdvancedFusionReactor extends RecipeMapMultiblockController implements IFastRenderMetaTileEntity, IBloomEffect {
    ...
    public MetaTileEntityAdvancedFusionReactor(ResourceLocation metaTileEntityId, int tier) {
        super(metaTileEntityId, ADVANCED_FUSION_RECIPES);
    ...
    protected BlockPattern createStructurePattern() {
        return FactoryBlockPattern.start()
                ...
                .where('E',
                        states(getCasingState()).or(metaTileEntities(Arrays
                                .stream(MetaTileEntities.ENERGY_INPUT_HATCH)
                                .filter(mte -> mte != null && tier <= mte.getTier() && mte.getTier() <= GTValues.UHV)
                ...
                .where('A', air())
                .where('I',
                        states(getCasingState()).or(abilities(MultiblockAbility.IMPORT_FLUIDS).setMinGlobalLimited(3)))
                .where('#', any())
                .build();
    }
    ...
    public List<MultiblockShapeInfo> getMatchingShapes() {
        ...
        MultiblockShapeInfo.Builder baseBuilder = MultiblockShapeInfo.builder()
                ...
                .aisle("######DCD######", "####GG###GG####", "######UCU######")
                .aisle("###############", "######EME######", "###############")
                .where('M', ADVANCED_FUSION_REACTOR, EnumFacing.SOUTH)
        ...
        /*shapeInfos.add(baseBuilder.shallowCopy()
                .where('K', getCoilState2())
                .build());

        shapeInfos.add(baseBuilder.shallowCopy()
                .where('K', getCoilState3())
                .build());

        shapeInfos.add(baseBuilder.shallowCopy()
                .where('K', getCoilState4())
                .build());*/
    ...
    private IBlockState getCoilState() {
        return GCYLRMetaBlocks.ADVANCED_FUSION_COIL.getState(BlockAdvancedFusionCoil.AdvancedFusionCoilType.ADVANCED_FUSION_COIL_MK1);
    }

    /*private IBlockState getCoilState2() {
        return GCYLRMetaBlocks.ADVANCED_FUSION_COIL.getState(BlockAdvancedFusionCoil.AdvancedFusionCoilType.ADVANCED_FUSION_COIL_MK2);
    }

    private IBlockState getCoilState3() {
        return GCYLRMetaBlocks.ADVANCED_FUSION_COIL.getState(BlockAdvancedFusionCoil.AdvancedFusionCoilType.ADVANCED_FUSION_COIL_MK3);
    }

    private IBlockState getCoilState4() {
        return GCYLRMetaBlocks.ADVANCED_FUSION_COIL.getState(BlockAdvancedFusionCoil.AdvancedFusionCoilType.ADVANCED_FUSION_COIL_MK4);
    }

    private IBlockState getCoilState5() {
        return GCYLRMetaBlocks.ADVANCED_FUSION_COIL.getState(BlockAdvancedFusionCoil.AdvancedFusionCoilType.ADVANCED_FUSION_COIL_MK5);
    }*/
    ...
    protected void formStructure(PatternMatchContext context) {
        ...
        Object type = context.get("AdvancedFusionCoilType");
        if (type instanceof IAdvancedFusionCoilStats) {
            this.coilTier = ((IAdvancedFusionCoilStats) type).getTier();
        } else {
            this.coilTier = BlockAdvancedFusionCoil.AdvancedFusionCoilType.ADVANCED_FUSION_COIL_MK1.getTier();
        }
    ...
    private class AdvancedFusionRecipeLogic extends MultiblockRecipeLogic {
        ...
        public boolean checkRecipe(@NotNull Recipe recipe) {
            ...
            // if the reactor's coil tier is not high enough, do not run the recipe
            if (recipe.getProperty(AdvancedFusionCoilTierProperty.getInstance(), 0) < coilTier)
                return false;
        ...
        protected void modifyOverclockPre(int @NotNull [] values, @NotNull IRecipePropertyStorage storage) {
            ...
            if (fusionTier != 0) fusionTier -= MetaTileEntityAdvancedFusionReactor.this.tier;
