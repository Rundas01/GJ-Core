...
import thegreggening.common.metatileentities.part.MetaTileEntityBeeHatch;
...
public class MetaTileEntityIndustrialAlveary extends RecipeMapMultiblockController implements IMachineHatchMultiblock {
    ...
    protected BlockPattern createStructurePattern() {
        return FactoryBlockPattern.start()
                ...
                .where('M', abilities(TGMultiblockAbility.BEE_HATCH).setExactLimit(1))
    ...
    public String[] getDescription() {
        return new String[] { I18n.format("tg.multiblock.mega_alveary.description") };
    }
        ...
        MultiblockDisplayText.builder(textList, isStructureFormed())
                ...
                .addCustom(t -> {
                    ITextComponent queensText = TextComponentUtil.translationWithColor(TextFormatting.GOLD,
                            "tg.machine.tooltip.ia_qc", Integer.toString(queenCount));
                    ...
                    ITextComponent maxQueensText = TextComponentUtil.translationWithColor(TextFormatting.GOLD,
                            "tg.machine.tooltip.ia_mqc", Integer.toString(finalMaxAmount));
                    ...
                    ITextComponent consumptionText = TextComponentUtil.translationWithColor(TextFormatting.WHITE,
                            "tg.machine.tooltip.ia_cc", Long.toString(consumption));
                    ...
                    ITextComponent cycleText = TextComponentUtil.translationWithColor(TextFormatting.WHITE,
                            "tg.machine.tooltip.ia_cycle", Integer.toString(Math.floorDiv(productionCycle, 20)));
    ...
    protected class MegaAlvearyRecipeLogic extends MultiblockRecipeLogic {
        ...
        public void updateQueen(RecipeMapMultiblockController controller) {
            ...
            if (isStructureFormed()) {
                ...
                ItemStack bee;
                for (int i = 0; i < controller.getAbilities(TGMultiblockAbility.BEE_HATCH).get(0).getSlots(); i++) {
                    bee = controller.getAbilities(TGMultiblockAbility.BEE_HATCH).get(0).getStackInSlot(i);
                    if (!bee.isEmpty()) {
                        queenCount += 1;
                    }
                }
    ...
    private void updateProductionCycle(RecipeMapMultiblockController controller) {
        ...
        if (isStructureFormed()) {
            ItemStack beeStack;
            for (int i = 0; i < controller.getAbilities(TGMultiblockAbility.BEE_HATCH).get(0).getSlots(); i++) {
                beeStack = controller.getAbilities(TGMultiblockAbility.BEE_HATCH).get(0).getStackInSlot(i);
                if (!beeStack.isEmpty()) {
    ...
    private void produce(RecipeMapMultiblockController controller) {
        ItemStack beeStack;
        IItemHandlerModifiable beeHatch = controller.getAbilities(TGMultiblockAbility.BEE_HATCH).get(0);
