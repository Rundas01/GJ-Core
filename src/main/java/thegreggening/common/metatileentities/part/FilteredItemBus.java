...
import thegreggening.api.metatileentity.part.FilteredItemStackHandler;
import thegreggening.api.metatileentity.part.IItemCondition;
...
public class FilteredItemBus extends MetaTileEntityItemBus {
    ...
    protected ModularUI createUI(EntityPlayer entityPlayer) {
        SlotWidget gsWidget = new GhostCircuitSlotWidget(circuitInventory, 0, 8, 100 + 18 * getTier())
        ...
        for (int i = 0; i < getTier(); i++) {
            ...
            for (int j = 0; j < getTier(); j++) {
                builder.widget(new SlotWidget(handler, i * getTier() + j, halfwidth + offset, 18 * (1 + i), true,
                        !isExportHatch)
