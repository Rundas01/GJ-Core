...
public class GJRecipeUtil {
    public static List<Integer> calculateMoles(List<Material> inputs, List<Material> outputs) {
        ...
        List<Material> distinctAtoms = Stream.concat(inputAtoms.stream(), outputAtoms.stream()).distinct().collect(Collectors.toList());
        ...
        for (Material atom : distinctAtoms) {
            ...
            for (Material material : outputs) {
                A[distinctAtoms.indexOf(atom)][outputs.indexOf(material) + inputs.size()] = (double) -getComponentAmount(material, atom);
            }
        }
    ...
    public static void registerMolaricRecipe(RecipeMap<SimpleRecipeBuilder> recipeMap, List<Material> inputs, List<Material> outputs, List<Material> fluidInputs, List<Material> fluidOutputs, int machineFactor, int itemFactor, int fluidFactor, int tier) {
        RecipeBuilder<SimpleRecipeBuilder> builder = getMolaricRecipe(recipeMap, inputs, outputs, fluidInputs, fluidOutputs, machineFactor, itemFactor, fluidFactor, tier);
        assert builder != null;
        builder.buildAndRegister();
    }

    public static RecipeBuilder<SimpleRecipeBuilder> getMolaricRecipe(RecipeMap<SimpleRecipeBuilder> recipeMap, List<Material> inputs, List<Material> outputs, List<Material> fluidInputs, List<Material> fluidOutputs, int machineFactor, int itemFactor, int fluidFactor, int tier) {
        if (inputs.size() > recipeMap.getMaxInputs()) {
            GJLog.logger.error("Cannot add recipe to " + recipeMap.getLocalizedName() + ". Too many item inputs!");
            return null;
        }
        if (outputs.size() > recipeMap.getMaxOutputs()) {
            GJLog.logger.error("Cannot add recipe to " + recipeMap.getLocalizedName() + ". Too many item outputs!");
            return null;
        }
        if (fluidInputs.size() > recipeMap.getMaxFluidInputs()) {
            GJLog.logger.error("Cannot add recipe to " + recipeMap.getLocalizedName() + ". Too many fluid inputs!");
            return null;
        }
        if (fluidOutputs.size() > recipeMap.getMaxFluidOutputs()) {
            GJLog.logger.error("Cannot add recipe to " + recipeMap.getLocalizedName() + ". Too many fluid outputs!");
            return null;
        }

        List<Material> totalInputs = Stream.concat(inputs.stream(), fluidInputs.stream()).collect(Collectors.toList());
        List<Material> totalOutputs = Stream.concat(outputs.stream(), fluidOutputs.stream()).collect(Collectors.toList());
        ...
        for (int i = 0; i < fluidOutputs.size(); i++) {
            builder.fluidOutputs(molaricFluid(fluidOutputs.get(i), moles.get(i + inputs.size() + fluidInputs.size() + outputs.size())));
        }

        builder.duration(calculateDuration(inputs2Calculate, fluidInputs2Calculate, machineFactor, itemFactor, fluidFactor));

        return builder;
    }

    public static void registerRecipe(RecipeMap<SimpleRecipeBuilder> recipeMap, List<ItemStack> inputs, List<FluidStack> fluidInputs, List<ItemStack> outputs, List<FluidStack> fluidOutputs, int machineFactor, int itemFactor, int fluidFactor, int tier) {
        RecipeBuilder<SimpleRecipeBuilder> builder = getRecipe(recipeMap, inputs, fluidInputs, outputs, fluidOutputs, machineFactor, itemFactor, fluidFactor, tier);
        builder.buildAndRegister();
    }

    public static RecipeBuilder<SimpleRecipeBuilder> getRecipe(RecipeMap<SimpleRecipeBuilder> recipeMap, List<ItemStack> inputs, List<FluidStack> fluidInputs, List<ItemStack> outputs, List<FluidStack> fluidOutputs, int machineFactor, int itemFactor, int fluidFactor, int tier) {
        RecipeBuilder<SimpleRecipeBuilder> builder = recipeMap.recipeBuilder().duration(calculateDuration(inputs, fluidInputs, machineFactor, itemFactor, fluidFactor)).EUt(VA[tier]);
        if (inputs != null) {
    ...
    public static int calculateDuration(List<ItemStack> inputs, List<FluidStack> fluidInputs, int machineFactor, int itemFactor, int fluidFactor) {
        double duration = 0;
        double duration2 = 0;
